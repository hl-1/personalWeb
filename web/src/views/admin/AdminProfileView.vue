...
watch(profile.data,(value)=>{if(value)Object.assign(form,{displayName:value.displayName,headline:value.headline,bioMarkdown:value.bioMarkdown,seoDescription:value.seoDescription,version:value.version})},{immediate:true})
...
async function save(){if(saving.value)return;saving.value=true;stale.value=false;errorMessage.value=undefined
  try{const saved=await props.adminClient.upsertProfile(parseProfileForm(form));Object.assign(form,saved)}
...
async function reload(){const result=await profile.refetch();if(result.data){Object.assign(form,result.data);stale.value=false}}
</script>
<template>
  <section>
    <header class="head">
