...
test('article create, safe preview, publish and stale-version handling', async ({ page }) => {
  ...
  await page.getByRole('textbox', { name: 'Title', exact: true }).fill('Safe article')
  await page.getByLabel('Slug').fill('safe-article')
  await page.getByLabel('Summary').fill('Summary')
  await page.getByLabel('Body', { exact: true }).fill('<img src=x onerror=alert(1)> **safe**')
  ...
  const createDialog = page.getByRole('dialog', { name: '文章添加成功' })
  await createDialog.getByRole('button', { name: '返回列表', exact: true }).click()
  ...
  await page.getByRole('textbox', { name: 'Title', exact: true }).fill('Preserved title')
  await page.getByRole('button', { name: 'Save', exact: true }).click()
  await expect(page.getByRole('alert').filter({ hasText: 'server version changed' })).toBeVisible()
  await expect(page.getByRole('textbox', { name: 'Title', exact: true })).toHaveValue('Preserved title')
})
...
test('taxonomy deletion reports reference conflicts', async ({ page }) => {
  ...
  const deleteDialog = page.getByRole('dialog', { name: '删除分类' })
  await deleteDialog.getByRole('button', { name: '删除', exact: true }).click()
  await expect(page.getByRole('alert').filter({ hasText: 'Remove article references first' })).toBeVisible()
})
